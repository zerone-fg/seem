...
import sys
sys.path.append("/mnt/paintercoco/xdecoder/body/decoder/")
import imgviz
import numpy
import torch
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, skip=None, up=None):
        if up is not None:
            x = self.up(x)
...
class MultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        nn.ConvTranspose2d(512, 512 // 4, kernel_size=2, stride=2),     #转置卷积 上采样2倍
        nn.BatchNorm2d(512 // 4),
        nn.LeakyReLU(inplace=True),
        nn.ConvTranspose2d(512 // 4, 512 // 8, kernel_size=2, stride=2),
        nn.BatchNorm2d(512 // 8),
        nn.LeakyReLU(inplace=True),
        nn.Conv2d(512 // 8, 10, 3, 1, 1)
    )
        self.decoder_1 = DecoderBlock(522+1034, 522, 522)
        self.decoder_2 = DecoderBlock(522+522, 522, 522)
    ...
    def forward(self, ref_information, query_information, mode='train'):
        ...
        for level_index in range(self.num_feature_levels):
            ...
            for j in range(2):
                ...
                bq, d, _, _ = src_mask_features.shape
                bs, d, _, _ = spatial_tokens.shape

                src_mask_features = src_mask_features.view(bq, d, -1).permute(0, 2, 1)
                spatial_tokens = spatial_tokens.view(bs, d, -1).permute(0, 2, 1)

                if mode == 'test':
                    # spatial_tokens = spatial_tokens.reshape(1, -1, d)
                    for i in range(spatial_tokens.shape[0]):
                        center_i = Spatial_DPC(spatial_tokens[i].detach().cpu())
                ...
                query_stage_list[level_index] = y.reshape(bq, self.spatial_list[level_index],
    ...
    def forward_prediction_heads(self, src, out_predict_list):
        ...
        ########## concat后进行卷积 ###############
        f_cat3 = self.decoder_1(f_3_aug, f_2_aug, up=True) #### (522, 64, 64)
        f_cat2 = self.decoder_2(f_cat3, f_1_aug)   ##### (522, 64, 64)
        ...
        outputs_mask = self.output_upscaling(
            torch.cat((f_cat2, out_predict), dim=1))
        # outputs_mask = self.output_upscaling(
        #     final
        # )
        results = {
