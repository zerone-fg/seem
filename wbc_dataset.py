...
def require_download_wbc():
    dest_folder = pathlib.Path("/mnt/universeg_wbc/")
    ...
    if not dest_folder.exists():
        ...
        os.makedirs(os.path.join(dest_folder, 'imgs'))
        os.makedirs(os.path.join(dest_folder, 'gts'))
...
if __name__ == "__main__":
    dataset = WBCDataset(dataset="JTSC", split="support")
    ...
    for idx in range(dataset.__len__()):
        ...
        img_1 = data[0].cpu().numpy()
        np.save(os.path.join("/mnt/universeg_wbc/imgs/", "wbc_{}.npy".format(idx)), img_1)
        
        gt_1 = data[1].cpu().numpy()
        np.save(os.path.join("/mnt/universeg_wbc/gts/", "wbc_{}.npy".format(idx)), gt_1)
