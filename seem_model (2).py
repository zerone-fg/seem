...
class SEEM_Model(nn.Module):
    ...
    def forward_seg(self, batched_inputs, targets, reference_img, ref_mask, area_list=None, mode='train'):
        ...
        area_avg =  1/ (area_list ** 2 + 1e-20)
        area_avg[area_avg == 1e20] = 1e-20
        area_avg = area_avg / area_avg.sum(dim=-1).unsqueeze(1)
        
        bs, channel, h, w = pred.shape
        pred = pred.view(-1, 1, h, w)
        targets = targets.view(-1, 1, h, w)

        area_avg = area_avg.view(-1)
        ...
        if self.task_switch['spatial'] and mode != 'test':
            if area_list != None:
                ...
                for id in range(area_avg.shape[0]):
                    losses += (self.criterion2(pred[id, 0, :, :].sigmoid(), targets[id, 0, :, :]) * 0.6 \
                                + self.criterion1(pred[id, 0, :, :], targets[id, 0, :, :]) * 0.4) * area_avg[id]
